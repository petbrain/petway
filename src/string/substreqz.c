...
[[nodiscard]] bool _pw_substring_eq_z(PwValuePtr a, unsigned start_pos, unsigned end_pos, void* b, uint8_t b_char_size)
{
    ...
    StrEqualZ fn_equalz = _pw_str_equalz_variants[char_size][b_char_size];
    int match = fn_equalz(a_ptr + start_pos * char_size, end_pos - start_pos, b);
...
[[nodiscard]] bool _pw_startswith_z(PwValuePtr str, void* prefix, uint8_t prefix_char_size)
{
    ...
    StrEqualZ fn_equalz = _pw_str_equalz_variants[str->char_size][prefix_char_size];
    int match = fn_equalz(str_start_ptr, length, prefix);
    return (match == PW_NEQ)? false : true;  // partial match means equal
}
...
[[nodiscard]] bool _pw_endswith_z(PwValuePtr str, void* suffix, uint8_t suffix_char_size)
{
    ...
    StrEqualZ fn_equalz = _pw_str_equalz_variants[str_char_size][suffix_char_size];
    int match = fn_equalz(str_start_ptr + (str_len - suffix_len) * str_char_size, suffix_len, suffix);
